...
import com.git.exception.token.RefreshTokenNotFoundException;
        ...
        if(StringUtils.hasText(token) && !jwtTokenProvider.validateExpiredToken(token)) {
            ...
            if(redisService.checkValues(redisRefreshToken)) {
                ...
                String accessToken = jwtTokenProvider.generate(subject,
                        new Date(now + AuthTokenGenerator.ACCESS_TOKEN_EXPIRE_TIME),
                        authentication);
                String refreshToken = jwtTokenProvider.generate(subject,
                        new Date(now + AuthTokenGenerator.REFRESH_TOKEN_EXPIRE_TIME),
                        authentication);
